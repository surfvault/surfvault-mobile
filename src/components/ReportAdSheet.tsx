...
import { useReportAdMutation } from '../store';
...
// Mirrors AD_REPORT_REASONS in surfvault-api/services/ad/handler.ts
export const AD_REPORT_REASONS: ReportReason[] = [
  ...
  { value: 'irrelevant', label: 'Not relevant to surfing' },
  { value: 'other', label: 'Other', description: 'Tell us what\'s wrong' },
];

const DETAILS_MAX = 500;

interface ReportAdSheetProps {
  visible: boolean;
  adId?: string;
  onClose: () => void;
}

export default function ReportAdSheet({ visible, adId, onClose }: ReportAdSheetProps) {
  const colorScheme = useColorScheme();
  const isDark = colorScheme === 'dark';
  const [report, { isLoading }] = useReportAdMutation();
  ...
  const handleSubmit = useCallback(async () => {
    ...
    Keyboard.dismiss();
    try {
      await report({ adId, reason: selected, details: details.trim() }).unwrap();
      onClose();
      Alert.alert(
        'Report submitted',
  ...
  }, [report, adId, selected, details, onClose]);
    ...
    >
      <SafeAreaView style={[s.flex, { backgroundColor: isDark ? '#0b1120' : '#ffffff' }]} edges={['top']}>
        <View style={s.header}>
          ...
          <Text style={[s.headerTitle, { color: isDark ? '#fff' : '#111827' }]}>Report Ad</Text>
          <Pressable
        ...
        >
          <View style={{ paddingHorizontal: 20, paddingTop: 8, paddingBottom: 16 }}>
            <Text style={[s.intro, { color: isDark ? '#9ca3af' : '#6b7280' }]}>
              Help us understand the issue. Your report is anonymous to the advertiser — our team
